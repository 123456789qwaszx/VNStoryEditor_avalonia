Phase 2 런타임 작업 순서 및 재개 지시서

2026-08-05 작성·최신화. VnTool 쪽 설계 확정(work-orders/phase2-design-brief.md §6,handoff/architecture-decisions.md H-2·H-3)과런타임 조사(core-extraction-survey.md)에 따른 실행 기록과 다음 작업용 프롬프트.

각 블록은 독립 커밋 단위다. U 단위로 커밋.

이 문서는 처음부터 순서대로 다시 실행하는 체크리스트가 아니다. U13-b-0부터 시작하라는 과거 지시는 폐기됐다.새 세션은 반드시 현재 저장소의 HEAD·커밋·테스트·최신 등가성 보고서를 먼저 확인한 뒤,아래 상태표에서 가장 앞선 실제 미완료 작업을 선택한다.

🚩 새 런타임 세션을 여는 것이라면 runtime-session-onboarding.md를 먼저 붙여넣을 것.이 문서는 작업별 프롬프트이고, 그쪽이 "왜·어디까지·무엇을"의 브리핑이다.

0. 현재 판정과 남은 관문 (2026-08-05 갱신)

✅ **Phase 2a 에셋 리허설: 소유자 판정으로 종료** (2026-08-05). `ExportedTuning/` 덤프 14파일 실재.

✅ **Phase 2b 착수 관문: 통과.** U13-a → U12-전체 → U13-b → U14 순서가 전부 실행됐고,
U14가 대표 에피소드(Story_blank_ch01_ep00) **193/193라인 등가** 초록불을 냈다 —
`EquivalenceReports/equivalence-20260805-211527.json` (Unhandled 153 보존).
⚠ 같은 날 늦은 두 리포트(22:05·22:06, Story_DungeonDinner)는 불일치로 남아 있으나, 선택지(`->`)가 있는
파일이라 하네스 범위(선형 파일 전용) 밖이다 — U14 판정에 넣지 않고, 갈래 인식(VnTool D-2b-4)의 백로그다.

**다음은 런타임이 아니라 VnTool 쪽이다**: `work-orders/phase2b-core-adoption.md` (W22–W26).
...
실제 HEAD에서 asmdef와 noEngineReferences: true를 재확인

U12-전체

런타임 덤프 구현 완료 보고

VnTool의 실데이터 수입은 별도 작업이며 아직 완료로 합치지 않음

U13-b-0~b-4

구현 완료 보고

현재 HEAD와 Core/EditMode 테스트로 재확인

U13-b-5

placement 14 → depth 7 → shot 5 → staging 12 이관 완료 보고

전체 201개 이관을 뜻하지 않음. 미지원은 Unhandled 유지

U14

**초록불 확인** (2026-08-05): 대표 에피소드 193/193 등가, `equivalence-20260805-211527.json`

portrait sizeDelta 불일치는 U14-c1~c4 수렴 커밋(d54456fc…3fe52e9b)으로 해소됨. 선택지 파일 리포트는 범위 밖(§0)

U12-v1

**완료 (소유자 보고, Phase 2a 종료에 포함)**

VnTool 규약 폴더 수입 확인까지가 완료 정의였다 — 소유자 판정

U14의 portrait 수정 후보 커밋으로 04040dfc가 언급됐지만, 이는 대화용 임시 저장소에서 만든커밋으로 알려져 있다. 실제 런타임 저장소나 GitHub에 있다고 가정하지 않는다. Unity 하네스 재실행결과도 아직 확인되지 않았으므로 U14 완료, Phase 2b 착수 가능이라고 기록하면 안 된다.

상태가 충돌할 때의 진실 우선순위는 다음과 같다.

실제 작업 저장소의 코드와 방금 실행한 테스트

현재 브랜치의 Git 커밋 이력

최신 마스터 플랜·architecture decision

이 문서의 과거 진행 보고

상태는 검증 완료 / 구현됐지만 필수 검증 미실행 / 과거 보고만 있고 현재 미확인 / 미구현으로나눠 기록한다. 코드가 있다는 이유만으로 검증 완료로 올리지 않는다.

1. U13-a — 어셈블리 경계 긋기 ✅ 완료 보고

소유자 예상: 코드를 신경 써서 만들었으니 순환 참조는 없고 경계만 그으면 된다.그래도 발견 건수는 보고하게 한다 — 0이면 0이라고 적으면 되고, 0이 아니면 그게 일정 신호다.
...
2. U12-전체 — 데이터 덤프 ✅ 구현 완료 보고

아래 프롬프트는 완료 범위 확인용 기록이다. 새 세션의 시작점이 아니다.
...
3. U13-b — 코어 추출 + 리듀서 표준화 ✅ 선정 범위 구현 완료 보고

U13-b를 한 덩어리로 지시하면 "커맨드 51개를 옮긴다"로 읽히는데, 실제 순서는 그렇지 않다.옮기기 전에 옮길 바닥(트랜스폼 수학 → 노드 트리 → 정착 계산)이 먼저 있어야 한다.아래 여섯 단계는 각각 독립 커밋이다. 현재는 선정한 4개 카테고리까지 구현됐다는 보고가 있으므로새 세션에서는 재구현하지 말고 HEAD와 테스트로 확인한다.
...
토큰 마무리 — 1u를 기준 폭 파생으로

완료 보고

b-1

값 타입 + 트랜스폼 수학 (유니티 대조 하네스 포함)

완료 보고

b-2

리그 노드 트리 모델 (StageState의 뼈대)

완료 보고

b-3

CharacterPlacementTargetLedger 승격 — 정착 계산 순수화

완료 보고

b-4

ClaimTarget 규약화 (타입 경계)

완료 보고

b-5

커맨드 이관 — placement/depth/shot/staging

선정 범위 완료 보고

b-0 — 토큰 마무리: 1u는 기준 폭 ÷ 48
...
CharacterPlacementTargetLedger가 부모를 타고 올라가며 InverseTransformPoint를 부른다.이걸 순수 계산으로 옮기지 못하면 그 위의 어떤 것도 못 옮긴다.⚠ RectTransform은 단순 TRS가 아니다 — anchorMin/anchorMax/pivot/sizeDelta와부모 rect 크기가 anchoredPosition → 실제 위치 계산에 들어간다.
...
이 단계는 VnTool을 위한 일이 아니라 런타임 자신의 견고함이다.지금 MeasureSettledWorldPoint는 정착 상태를 알기 위해 유니티를 잠깐 오염시켰다 되돌리는데,try/finally가 없어 측정 중 예외가 나면 리그가 더럽게 남는다.
...
착수 전에 표본 5~10개를 읽고 편차를 보고할 것. 조사는 51곳의 개수만 셌다(조사 §6-3).편차가 크면 여기가 U13-b의 대부분이고, 작으면 b-5가 기계적 반복이 된다.
...
4. U14 — 등가성 하네스 수렴 ✅ 초록불 (2026-08-05, §0 참조)

아래 프롬프트는 재수렴이 필요해질 때(코어 리듀서 추가 이관·회귀 의심)의 기록이다. 새 세션의 시작점이 아니다.

런타임 저장소의 U14 등가성 관문을 이어서 수렴시켜줘.
근거: runtime-work-orders.md의 U14, core-extraction-survey.md §3,
phase2-runtime-orders.md의 최신 상태표.

중요: U13-b-0부터 다시 시작하지 않는다. U14가 완료됐다고 가정하지도 않는다.
먼저 실제 저장소의 브랜치·HEAD·git status·최근 U13/U14 커밋·최신 EquivalenceReports를 확인하고,
다음 네 상태로 재개 보고서를 작성한다.
  - 검증 완료
  - 구현됐지만 필수 검증 미실행
  - 과거 보고만 있고 현재 저장소에서 미확인
  - 미구현

마지막으로 알려진 기준선은 대표 에피소드 193/193라인의 portrait sizeDelta 불일치다.
다만 이 수치와 수정 후보가 현재 HEAD에도 그대로라고 추정하지 말고 실제 보고서로 재현한다.
`04040dfc`는 대화용 임시 저장소에만 있었던 것으로 알려진 커밋이므로 존재를 가정하거나
그 해시를 cherry-pick하지 않는다. 실제 diff를 읽고 필요한 변경만 다시 판단한다.

portrait 불일치 조사에서 확인할 축:
1. `pose` 디스패치와 variant 상태 추적
2. 인자 없는 `show`의 기본값 `e1`
3. 표정별 portrait 치수 조회와 정규화
4. clone 이후 portrait 상태의 독립성
...
수용 기준:
- 지원한다고 선언한 placement/depth/shot/staging 비교 축이 전 라인에서 허용 오차 안에 일치한다.
- 새 불일치 클래스가 생기지 않는다.
- 미지원 커맨드는 이름·출처·이유와 함께 `Unhandled`에 남는다.
- 배경·대사창·화면 효과·오디오 등 아직 비교하지 않는 축까지 등가라고 부르지 않는다.
- 실제 Unity 하네스 결과와 생성 시점이 남는다.

Unity를 실행하지 못했다면 "구현 완료, U14 검증 대기"로 보고하고 "U14 완료"라고 하지 않는다.
하네스를 느슨하게 만들어 맞추지 말고, 불일치는 리듀서·호스트 어댑터·캡처 중 실제 원인을
찾아 최소 변경으로 수렴시킨다. 커밋은 원인 클래스 하나당 하나로 나눈다.

5. U12-v1 — 초상화 PNG 규약 내보내기 (독립 미완료 트랙)

U12-v1은 U14와 독립이며 Phase 2a의 실제 에셋 리허설에 즉시 가치가 있다. 현재 exporter가portrait 치수 JSON을 내보낸다는 사실만으로 완료 처리하지 않는다. 완료하려면PortraitGeneratedDbSo.entries[]의 실제 PNG가 다음 규약 경로로 복사돼야 한다.

{portraitsRoot}/{characterId}/{variantKey}/{emotionKey}.png

파일·폴더명은 PortraitKey에서 기계적으로 조립한다.

유사 이름 추측·자동 교정은 금지한다.

표현할 수 없는 엔트리는 조용히 버리지 말고 경고 목록에 남긴다.

VnTool은 규약 폴더 스캔이 1순위다. 매니페스트는 권위가 아니다.

내보낸 폴더를 VnTool 에셋 루트에 두고 JSON 수작업 없이 해석되는지 확인해야 완료다.

상세 프롬프트는 runtime-parallel-orders.md §3을 사용하되,현재 HEAD에 이미 동등한 구현이 있다면 재작성하지 말고 실제 PNG 출력과 수입으로 검증한다.
