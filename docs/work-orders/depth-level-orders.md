...
## 8. 회신 — 개통 완료 (2026-08-21)

그쪽이 ①②를 넣으면서 **한 걸음 더 갔다**: 프리셋 표를 버리고 라벨을 레벨 커브 위의
눈금으로 통일했다(`DepthLevelLabels` — far 0 · back 2.5 · mid 5 · front 7.5 · close 10 ·
exp1 12 · none·exp2 = mid). 재생과 폴드가 같은 커브·같은 눈금표를 지난다.

이쪽 반영 끝: 코어 사본 동기화(6파일 + `DepthLevelLabels.cs`, 공유 테스트 포함) ·
튜닝 로더가 `level`을 싣는다 · 작업대 입력이 **슬라이더 하나 + 눈금 라벨**로 합쳐졌다.
실측으로 `mid ≡ 5`, 구간 밖(-3.5 · 12.5) 외삽, 헛토큰 거부까지 확인했다.

**§5 수용 기준** — 1·2·3·5 충족. **4번(프리셋 회귀 없음)은 그쪽 결정으로 폐기**됐다:
라벨 재생값이 바뀌는 것이 이 통일의 대가이고, 그 대가를 알고 택했다.

**남은 것은 눈금 조정 판단 하나** — 코퍼스가 mid 6회·front 2회를 쓰고 mid의 y가
0 → -160으로 내려간다. 에디터 육안 확인 뒤 옮기고 싶으면 `DepthLevelLabels` 상수 하나다.
