...
# 증보 ① — 표준 이징을 핑퐁으로 받는다 (2026-08-21, 소유자 요청)

> **소유자** — "ease를 기존 moveBy나 depth place처럼 기본적으로 DoTween을 포팅한 ease를
> 쓰고, 원한다면 **즉석에서 바로 커스텀**하고 싶은데 그건 안될까? 지금처럼 **키를 불러오는
> 방식은 불편**해."

## 왜 지금 불편한가

지금 `gesture`의 곡선 자리는 `@이름`만 받는다. 표준 이징 이름을 적으면 브리지가 거부한다
(`ResolveOscillationCurve` — "끝값이 1이라 제자리로 안 돌아온다"). 그래서 흔들기 하나를
쓰려 해도 **먼저 곡선을 만들어 이름을 붙여 두는** 준비 과정이 필요하다. move_by·place·size가
`OutBack` 한 낱말로 끝나는 것과 대비가 크다.

## 해법 — 핑퐁(ping-pong)

표준 이징은 0→1이라 그대로는 못 쓴다. 하지만 **나갔다 돌아오는 왕복의 절반**으로 읽으면
그대로 진동이 된다:

```
PingPong(ease, t) = ease( t < 0.5 ? 2t : 2 - 2t )
```

`t=0`에서 0, `t=0.5`에서 1(최대 변위), `t=1`에서 0 — **순변위 0이 그대로 지켜진다.**
그리고 35종이 전부 서로 다른 몸짓이 된다:

| 이징 | 핑퐁하면 |
|---|---|
| `OutQuad`·`OutCubic` | 튕겨 나갔다 돌아오는 **총총(hop)** |
| `OutBack` | 끝에서 살짝 넘겼다 오는 **오버슈트 흔들림** |
| `OutBounce` | **통통** 튀는 착지 |
| `OutElastic` | 떨리는 **shake** |
| `Linear` | 각진 **삼각파 — 날카로운 jolt** |
| `OutSine` | **지금 기본 혹 `sin(πt)`과 정확히 같은 함수** — 아래 정정 참조 |

> **정정 (2026-08-21, 런타임 실측)** — 위 표에 처음 `InOutSine`을 기본 혹과 "거의 같다"고
> 적었는데 **틀렸다**. 실측 최대차 0.214다(t=0.25에서 0.5 vs 0.707). 기본 혹과 같은 것은
> **`OutSine`**이고 오차 0으로 <b>수식상 동일</b>하다:
> `PingPong(OutSine, t) = sin(2t · π/2) = sin(πt)`.
>
> 이건 좋은 소식이다 — **기본 혹이 특별한 무엇이 아니라 `OutSine` 핑퐁의 별칭**이라는 뜻이라
> 두 체계가 정확히 이어진다. 선택기에서 `OutSine`을 고르면 기본값과 한 치도 안 다르고,
> 거기서 [곡선 편집…]으로 넘어가면 **기본 혹이 키로 펼쳐진 채** 열린다.

## 계약 변경

**인자 문법은 그대로다.** `xEase`/`yEase`가 받는 것이 하나 늘 뿐이다:

| 적은 값 | 뜻 |
|---|---|
| 빈 토큰 | 내장 기본 혹 `sin(πt)` (지금 그대로) |
| `@이름` | 진동 곡선 (지금 그대로) |
| **`OutBack` 등 표준 이징 이름** | **그 이징의 핑퐁** ← 새로 받는 것 |

```
<<gesture c1 0.3u 0u 12fr OutElastic>>        ; 떨리는 흔들기 — 준비 과정 없음
<<gesture c1 0u 1.2u 24fr "" OutBounce>>      ; 통통 튀는 총총
<<gesture c1 0u 1u 18fr "" @my_hop>>          ; 손으로 그린 곡선(지금 그대로)
```

## 부탁 — 런타임에서 할 일

1. **코어** `OscillationFunctions`에 한 함수:
   ```csharp
   public static float PingPong(EaseKind kind, float t)
       => EaseFunctions.Evaluate(kind, t < 0.5f ? t * 2f : 2f - (t * 2f));
   ```
   툴 프리뷰가 이 함수를 그대로 쓴다 — 정본이 하나여야 "보이는 모양 = 재생하는 모양"이다.

2. **스펙** `GestureCommandSpecCharR`에 축별 이징 두 칸(`xEase`·`yEase`, `EaseKind?`).
   곡선 키가 있으면 키가 이기고, 없고 이징이 있으면 핑퐁, 둘 다 없으면 기본 혹.

3. **브리지** `ResolveOscillationCurve`의 마지막 거부 갈래를 **파싱으로 바꾼다** —
   표준 이징 이름이면 `EaseKind`로 넘긴다. 못 읽는 낱말만 경고 + 기본 혹으로 남긴다
   (침묵 금지는 그대로).

4. **테스트**: `PingPong` 끝점이 0·0이고 중앙이 1 · 이징별 중간값이 `EaseFunctions`와 일치 ·
   `<<gesture c1 0u 1u 24fr "" OutBounce>>`가 경고 없이 접히고 재생된다.

## 즉석 커스텀은 이 변경으로 저절로 열린다

툴의 이징 선택기(`BuildEaseSelector`)는 이미 **표준 이징 목록 + 실시간 곡선 미리보기 +
[곡선 편집…]**을 갖고 있고, 그 버튼이 하는 일이 "**선택한 이징을 5키로 베이크해 출발**"이다
(W67). gesture가 표준 이징을 받게 되면 그 선택기를 그대로 붙일 수 있고, **핑퐁을 베이크하면
끝점이 (0,0)·(1,0)이라 그 자체로 유효한 진동 곡선**이다 — 두 체계가 이어진다.

즉 작가의 길이 이렇게 된다: `OutBack`을 고른다 → 마음에 안 들면 [곡선 편집…] → 그 모양이
키로 펼쳐진 채 열린다 → 손보고 `@이름`으로 저장. **준비 과정이 사라진다.**

## 열어 둔 결정 하나

빈 토큰의 기본을 지금대로 `sin(πt)`로 둘지, 다른 커맨드와 맞춰 **핑퐁(OutCubic)**으로 바꿀지는
소유자 결정이다. 이쪽 의견은 **지금대로 유지** — `sin(πt)`가 가장 둥글고 중립적인 몸짓이고,
핑퐁(OutCubic)은 가운데가 뾰족해 기본값으로는 성격이 세다. 원하면 `OutCubic`을 적으면 된다.

## 대안으로 검토했다 접은 것

**감쇠형** `ease(t) × (1-t)` — 부풀었다 사그라드는 결. 끝값 0은 만족하지만 최대점이 앞으로
쏠려 "총총"보다 "훅 불었다 꺼짐"에 가깝다. 핑퐁이 요청한 용도(hop·shake·jolt)에 더 맞아
1안으로 두되, 나중에 둘째 모드가 필요하면 같은 자리에 붙일 수 있다.
