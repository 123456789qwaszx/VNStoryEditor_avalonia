...
## W24 — 폴드 교체 (화면·출력 불변 골든) ✅ 완료 (`664ba7c`, 723 테스트)

`CoreStageFold` 합성 폴드 + 골든 8종 초록. 지시서에서 벗어난 결정 둘(커밋 메시지에 기록):
① 구 폴드의 중복 축을 지우지 않았다 — tuning 없는 프로젝트의 폴백이 그 자리다.
② 표정 키는 코어 정규화 어휘("02")를 채택 — 초상 해석과 같은 어휘라 화면 결과 동일, 골든은 정규화 위 비교.
...
## W25 — 좌표 배치: 팝오버 저작이 실제로 동작한다 ✅ 완료 (`8bfc1de`, 728 테스트)

place·size·이동·샷이 코어 좌표로 실제 배치된다. 팝오버 안내문 교체, "좌표 근사(tuning 없음)"
뱃지, 코어 밖 슬롯 균등 나열 보존. **의도적 범위 제외**: 대사창 surface-layout 재현(기존 근사+뱃지
유지 — 스키마 검토 후 별도 단위), 뎁스 그리기 순서(슬롯 키 순서 유지).
**남은 수용 절차**: 유니티 스크린샷과의 구도 대조(수동, 소유자 판정) — 아래 원문 수용 기준 참조.
...
## W26 — 근사·미표시 뱃지 (H-3 마감) ✅ 완료 (`0c10e84`, 729 테스트)

"반영 안 된 연출 N"(코어도 툴도 못 접음) / "미표시 N"(접혔지만 안 그림 — char_to·이후 셰이더 축)
두 뱃지로 분리, 코어 진단(초상 치수 없음 등)이 상세 목록에 사유·출처와 함께 노출된다.
...
## W27 — 슬롯 위치(무대/레이어) 실반영 + 깊이 UI ✅ 완료 (`cbe2333`, 731 테스트)

W26 이후 소유자 지시(2026-08-05)로 추가된 단위.

- **그리기 순서 = 슬롯 부착 (stage, layer, 슬롯 키)** — slot/char_to가 기록해 온 stage·layer
  인자가 처음으로 화면 의미(앞뒤 겹침)를 갖는다. far(뒤)→close(앞), 무대는 Ordinal 오름차순.
  레이어 어휘·순서의 원천은 `ArgumentTokenCandidates`의 layerKey 후보 한 곳.
- **슬롯 탭에 무대/레이어 콤보** — 슬롯 생성 시 위치가 slot 커맨드 인자로 기록된다.
- **깊이(size) 버튼 행**(far~close)을 슬롯 탭·캐릭터 팝오버가 공용 — 이 라인의 `size`가 되고
  코어 depth 프리셋이 스케일·높이를 실반영한다. 같은 슬롯 재선택은 커맨드 수정(dedupe).
- W26의 "미표시"에 남는 char_to는 이제 레이어 변경 시 순서로 반영된다 — 단 v1 컨테이너
  항등 가정은 유지(좌표 이동 없음).

## W28 — 조절창 UX 재구성 ✅ 완료 (`f9a8dec`, 735 테스트)

소유자 지시(2026-08-06). 조절창은 적용해도 닫히지 않고 우클릭이 닫는다(공통 틀
`ShowManipulationFlyout`). 슬롯 추가([이름]+[+])와 선택(드롭다운 + 강조 칩)은 탭 밖
전역 헤더로. 슬롯 탭 = 선택 슬롯의 무대/레이어(현재 값 표시·즉시 적용)·위치·깊이(버튼
격자 유지)·우하단 등장(초록)/퇴장(붉음). 숨김·빈 슬롯은 프리뷰에 점선 윤곽 + 슬롯명
태그(고스트)로 자리가 보이고 클릭으로 조작한다.
