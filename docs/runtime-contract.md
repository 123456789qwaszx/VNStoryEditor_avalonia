...
### D1-1. 대사가 스탯을 **읽는** 표기 — `stat("키")` (2026-09-17, ⏳ 양쪽 대기)
...
| 저쪽 | ⏳ 같은 이름의 Yarn 함수를 등록한다. 인자는 스탯 키 문자열, 반환은 정수. 깃발은 0/1 |

⚠ **그때까지 내보내면 번들이 컴파일에서 선다.** 함수가 없기 때문이다 — 조용히 빗나가는
것보다 나은 실패지만, 저쪽 반영 전에는 export를 미루는 것이 맞다.
...
**G-5. `Option.ViaNodeId` 내보내기 — ✅ 2026-08-19 완료. ⚠ 2026-09-17에 저작 쪽이 이 칸을
쓰지 않게 됐다 (R7 P-6).** 간선의 `연출` 칸이 `NextOptions[].ViaNodeId`로 나간다. 자동 진행
간선에도 붙고, 없으면 빈 문자열로 언제나 낸다. ⚠ 저작 이름(`PresentationNodeName`)과 계약
이름이 갈리는 자리라 **키 이름을 글자 그대로** 붙드는 테스트를 걸었다 — 틀리면 역직렬화기가
조용히 버려 오류 없이 연출만 사라진다.

> **⛔ 계약은 안 바뀌었다. 칸은 그대로 있고 규격도 그대로다.** 바뀐 것은 저작 쪽이
> **그 칸을 채우지 않게 됐다**는 것뿐이다 — "길에 매단 연출"을 같은 재생 순서의
> **에피소드 한 칸**으로 펴기 때문이다(`A —문구→ 연출 —자동→ B`). 계약이 *"없으면 빈
> 문자열로 언제나 낸다"*고 이미 적고 있으므로 런타임은 **한 줄도 안 고쳐도 된다.**
>
> 툴은 판을 열 때 옛 배선을 그 모양으로 편다(`ProjectEditor.LiftViaScenes`). 이름이 이미
> 다른 에피소드에 쓰이고 있는 자리는 **일부러 안 편다** — 조용히 개명하면 어느 씬이 어디로
> 갔는지 알 수 없기 때문이고, 그런 자리는 옛 길로 계속 나간다. 그래서 산출물에서 **연출이
> 사라지는 일은 없다.**
>
> **칸 자체를 걷는 것(P-6 c)은 아직 안 했다** — `Ked.Progression`은 런타임 코어의 사본이고
> `runtime-sync.txt` 지문이 둘을 묶는다. 이쪽에서만 걷으면 **대조가 깨지고 런타임이 로드에서
> 죽는다.** 걷는 시점은 저쪽 일정과 함께 정한다.
...
**H-3. `Option.ViaNodeId` — ✅ 2026-08-19 완료. ⚠ 다시 소유자에게 왔다 (2026-09-17).**
...
> **정할 것: 이 칸을 걷을 것인가 (R7 P-6 c).** 저작 쪽은 2026-09-17부터 **이 칸을 안 쓴다**
> — 길에 매단 연출을 같은 순서의 에피소드 한 칸으로 펴기 때문이다(§G-5). 산출물에는 늘 빈
> 문자열로 나가므로 **안 걷어도 아무 문제가 없다.**
>
> 걷으려면 **양쪽이 함께** 움직여야 한다: `Ked.Progression`은 런타임 코어의 사본이고
> `runtime-sync.txt` 지문이 둘을 묶어, 한쪽만 고치면 대조가 깨지고 런타임이 로드에서 죽는다.
> 서두를 이유는 없고, 남겨 두는 비용은 **늘 빈 칸 하나**다.
