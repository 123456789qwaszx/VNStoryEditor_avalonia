...
> ⛔ **추기 (2026-08-26, v14)** — 엔딩키가 모양을 바꿨다. 둘로 나뉜다:
>
> ① **`EndingKey`·`IsChapterEndingCandidate`가 다시 안 나간다.** 저작에서 간선의 엔딩키가
> 폐지됐다(소유자: 툴의 보장은 챕터 안의 분기·그래프 구조까지 — 어차피 `EndingRules`가
> 언제나 빈 배열이라 키를 읽는 곳이 없었다). **그쪽 DTO·로더는 안 바꿔도 된다** — 두 칸 다
> 빈 값이 기본이고, 로더의 일관성 검사(§2의 충돌 거부 포함)는 값이 없으면 그냥 조용하다.
> 아래 본문의 엔딩키 서술(§0 표 첫 줄 · §2 · §5 예시)은 이제 역사다. 시나리오 층(챕터를
> 잇는 판)을 지을 때 엔딩 식별을 그쪽 규격과 함께 다시 정하자(H-4와 같은 날).
>
> ② **부탁 하나 — 노드에 `EventKey`(string) 패스스루 칸.** 에피소드 시트의 새 `이벤트키`
> 열이 노드 `EventKey`로 실려 나가기 시작했다. 뜻: **"이 에피소드를 다 시청했을 때"**
> 유니티가 자체적으로 이벤트·보상을 발생시키는 인덱스(소유자 정의). 진행 평가에는 안
> 끼고, VnTool도 해석하지 않는다 — `DesignerNote`와 같은 결의 화물이다. 지금은 그쪽 DTO에
> 칸이 없어 **역직렬화가 조용히 버린다** — `EpisodeNodeDto`에 string 하나, `EpisodeNode`에
> 그대로 얹어 주면 호스트가 읽을 수 있다. `StatChange.Op` 때와 같은 절차인데, 그때와 달리
> 값이 오해될 자리가 없어(없으면 그냥 없음) 이쪽이 먼저 싣는다.
>
...
| ~~깃발을 실을 `StatChange.Op`~~ | ✅ **닫힘 (2026-08-23)** — 그쪽 `0.2.0`에 칸이 섰고 이쪽이 실었다 |
| **⛔ `Option.Kind`를 읽을 칸** | **새 부탁 하나 (§6-1)** — 이쪽은 이미 내고 있다 |

---

## 0-1. 4차에서 바뀐 것 — 2026-08-23
...
| **⚠ `DialogueEntryId`의 모양이 바뀌었다** | `대사엔트리`가 **글자 그대로 안 나간다.** `SanitizeNodeName(이름)`을 통과한다(`장면 1` → `장면_1`). 2026-08-24까지는 `"Story_"` 접두도 붙었으나 폐지됐다(중복으로 붙고 있었다). 코어는 이 값을 문자열로만 다루므로 **그쪽 코드는 안 바뀐다** — 다만 픽스처를 다시 뜨면 값이 달라진다(§0-2) |
...
`ked-progression/Tests/Fixtures/chapter-ch01-sample.json`은 이쪽
`docs/ch01.progression.sample.json`의 **사본**인데, 이쪽을 다시 구우면서 갈렸다:

```
이쪽 골든    "DialogueEntryId": "시작"          + "Op" + "Kind"
저쪽 픽스처  "DialogueEntryId": "시작"          (칸 둘 없음)
```

⚠ **2026-08-24에 `Story_` 접두가 폐지되면서 이 칸의 값은 도로 같아졌다** — 남은 차이는
`Op`·`Kind` 두 칸뿐이다.

그쪽 테스트는 `EpisodeId`만 붙들어서 **깨지지 않는다.** 다만 그 표본은 "툴이 실제로 내는
모양"을 보여 주기로 한 파일이고, 지금은 그렇지 않다. **파일 하나 복사면 끝난다** —
저장소 경계를 넘으므로 이쪽에서 임의로 밀지 않았다.

---

**v11 규격 6단계가 전부 닫혔고, 깃발도 닫혔다.** 남은 경계면은 `Option.Kind` 하나다.
...
### 6-1. ⛔ 부탁 — `EpisodeOptionDto`에 `Kind` 칸 하나
...
저작 데이터에 종류 열이 없어 문구의 유무로 판별한다(D5).
```

**저작 데이터에는 이제 종류 열이 있다.** 칸 하나를 세우면 코어가 추론을 그만두고, 지금은
...
> ✅ **닫혔다 (2026-08-23)** — 아래는 왜 거부까지 했는지의 기록이다. `ViaNodeId`처럼 "저작에만 살고
...
VnTool이 내던 트리오(`Story_*` / `Set_*` / `Pres_*`)가 **`{대사엔트리}.yarn` 하나**로
