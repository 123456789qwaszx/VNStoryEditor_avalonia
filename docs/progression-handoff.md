...
기준: 2026-08-18 (**3차**) · VnTool(`java-start`) 저장소 · 테스트 1180 통과
...
| **`Option.ViaNodeId`** | ✅ **나가기 시작했다** (v11 §6 — 이번) |
...
`Initial`에 영원히 묶여 있다.** 조건에서 쓸 수는 있지만 값이 변할 방법이 없다. 규격을
정하면 저작·계약 양쪽이 함께 움직여야 한다.
