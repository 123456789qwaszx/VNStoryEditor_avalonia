...
기준: T2 수동 검증 완료 시점 (2026-08-05) · 테스트 **487개 전원 통과** (Core 60 / Authoring 363 / App 64) · 작업 트리 clean.
...
| 고아 출력 가시화 (T3, K1 ②안) | `afbbcc6` | `OutputManifest` — 낡은 산출 파일을 보여 주되 지우지 않는다 |
| 저작 UX 보강 | `3d31752`…`cfced8e` | 고밀도 대사 편집 · 무대 팝오버 3탭 · 캐릭터 팝오버 확장 · 표정 스프라이트 복제 · 화자 입력 수정 · 선택지 제시 프리뷰 |
...
- **선택지 제시 근사**: 선택 라인이 옵션 라벨이면 대사창 대신 그 블록의 옵션 전부가 화면 중앙에
  버튼 묶음으로 나온다(선택된 라벨은 노란 테두리). 판정은 `ChoiceOptionBundle` 하나 —
  작업 대본과 발행 결과 프리뷰가 공유.
- 연출노드에서는 무대 직접 조작 가능, 대사노드(발행본 열람)에서는 잠기고 이유가 뜬다.

**에셋**
- **표정 정의 + 스프라이트 복제**(설정노드 화자 행 [표정…]): variant·표정번호를 정하고 규약 파일이
  없으면 PNG를 골라 `{root}/{char}/{variant}/{emotion}.png`로 **복제**해 즉시 등록
  (`PortraitSpriteImporter` — 원본 보존, 기존 파일 덮어쓰기 거부, PNG 외 변환 없이 거부).
  초상화 폴더는 참고용이 아니라 **복제본을 모으는 자리**다 — 있으면 그냥 쓴다.
...
- **Phase 2a/2b/2c** — 지금은 **설계 단계**다. 범위·의존·미결 질문은
  [work-orders/phase2-design-brief.md](../work-orders/phase2-design-brief.md).
  D-2b-1(리듀서를 런타임 코어와 공유할 것인가)이 답해지기 전에는 2b 코드를 쓰지 않는다.
- **VnTool 단독으로 착수 가능한 2단계 작업은 없다** — 2a는 런타임 U12-전체가, 2b는 U13·U14가 입력이다.
- 프리뷰가 접는 것은 카탈로그 201개 중 **20여 종**뿐이다. 나머지는 `Unhandled`로 쌓여 있고
  그것이 2b의 확장 백로그다. 좌표·뎁스·카메라는 아직 상태에 없다.
