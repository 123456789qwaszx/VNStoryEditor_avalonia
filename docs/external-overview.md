...
> **⚠ `ViaNodeId`는 2026-09-17(R7 P-6)부터 저작 쪽이 채우지 않는다 — 늘 빈 문자열이다.**
> 「길에 매단 연출」을 같은 재생 순서의 **Episode 한 칸**으로 펴기 때문이다:
> `A —문구→ 연출 —자동→ B`. 런타임이 A를 틀고, 고른 뒤 연출을 틀고, 묻지 않고 B로 가므로
> **순서가 같다.** 곁칸은 같은 것을 말하는 두 번째 방법이었고, 같은 것이 두 곳에 살면 갈린다.
>
> **계약과 로더는 안 바뀐다.** 칸도 규격도 그대로이고, 계약이 이미 *"없으면 빈 문자열"*을
> 말한다. 칸 자체를 걷는 것은 사본 지문(`runtime-sync.txt`)이 양쪽을 묶고 있어 **함께**
> 해야 하는 일이라 미뤄 뒀다(계약서 §H-3).
