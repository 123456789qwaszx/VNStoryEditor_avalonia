...
| 값 | 원본 |
|---|---|
| 에피소드·간선·선택지·조건·스탯 | `chapters/{id}.xlsx` — ⚠ **간선은 [연출 그래프]에서도 긋는다**(2026-09-17, R7): 카드 아래 3칸을 끌어다 놓으면 `ChapterDocument.Edges`를 그대로 고친다. **창구가 둘이지 정본은 하나**다 |
| 대사 본문·화자·라인 구조 | `episodes/{chapter}/{episode}.xlsx` |
| Yarn 진입 이름·인라인 연출 | 연출 그래프 |
...
| Chapter | 확정 진행 스탯 (⚠ Yarn 변수는 2026-09-17에 폐지 — §6.5) | 챕터 시작 시 초기화·복원 |
...
Option
  ...
  LockedReasonText, StatChanges[], Auto
...
`DialogueEntryId`는 Episode 본문 Yarn node다. 이름의 주인은 연출 그래프다. 엑셀에 중복 소유시키면 node 개명 때 JSON과 Yarn이 갈라진다. 산출 검증은 대상 Yarn node의 실재까지 확인해야 한다.

> ⛔ **`ViaNodeId`는 2026-09-17에 양쪽에서 폐지됐다.** 「길에 매단 연출」을 같은 재생
> 순서의 **Episode 한 칸**으로 말할 수 있기 때문이다: `A —문구→ 연출 —자동→ B`. 곁칸은
> 같은 것을 말하는 두 번째 방법이었고, 같은 것이 두 곳에 살면 갈린다.
>
> 옛 JSON에 `"ViaNodeId": ""`가 남아 있어도 로더가 모르는 필드를 무시하므로 안 깨진다.
> 상세는 계약서 §G-5.

### 6.5 스탯은 한 계층이다 (2026-09-17 개정)

⛔ **옛 서술은 두 계층이었다** — 진행 스탯과 Yarn 변수. 그 구분이 폐지됐다.
**작가 전용 변수라는 개념이 양쪽에서 사라졌고**, 대사가 물을 수 있는 낱말은 챕터 스탯
하나뿐이다. 런타임은 Yarn 변수 층 전체를 걷었다.

- **읽기**: 대사에서 `<<if stat("키") >= 3>>` — 읽기 전용 **함수**다(계약서 §D1-1)
- **쓰기**: 챕터 간선의 `스탯변화` **하나뿐**. `<<set>>`은 한 줄도 안 나간다(§D1-2)

변수가 아니라 함수인 이유가 여기 있다 — ``로 심으면 쓰기가 문법적으로 유효해지고,
대사 중의 스탯 쓰기는 세이브/로드 복귀와 도달성 증명이 못 보는 뒷길이 된다. 함수에는
왼쪽 변이 없다: 금지가 아니라 불가능이다.

Bool은 `Equal 0/1`과 `Set 0/1`, Number는 `Add`만 허용한다.
...
| `EventKey` 출력 | 정상 |
| ~~연출 그래프를 원본으로 하는 `ViaNodeId`~~ | 2026-09-17에 칸이 폐지됐다 (§6.4) |
...
- 진행 스탯을 `stat("키")` 함수로 읽었는가? (`$변수`는 막힌다)
- `<<set>>`을 한 줄이라도 내지 않았는가? (롤백에서 안 되감겨 리플레이가 멈춘다)
