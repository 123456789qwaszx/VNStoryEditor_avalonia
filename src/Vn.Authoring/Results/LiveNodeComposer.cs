...
public static class LiveNodeComposer
{
    ...
    /// 나가고, 유니티의 사전 대조가 "부르는 노드가 YarnProject에 없다"로 <b>재생을 통째로
    /// 막는다</b>. (2026-09-17까지는 간선에 매단 <c>ViaNodeId</c>가 그 모양의 대표 사례였다.
    /// 그 칸은 걷혔지만, 에피소드 이름으로 부르는 구조는 그대로다 — 이유가 안 바뀌었다.)
    ///
    /// 빈 노드는 들어갔다 곧장 나오면 그만이다. 비어 있다는 사실은 <b>이 경고가</b> 말한다 —
    /// 산출물에서 조용히 빠지는 것보다 언제나 낫다.
    ///
    /// ⛔ 옛 머리글은 챕터 그래프의 <c>ViaSceneEmpty</c> 경고를 함께 근거로 댔다. <b>그
    /// 진단은 없다</b> — `ViaNode`와 함께 걷혔다. 없는 검사를 근거로 적어 두면, 다음 사람이
    /// 두 겹으로 막혀 있다고 믿고 이쪽을 마음 놓고 걷는다.
    /// </summary>
