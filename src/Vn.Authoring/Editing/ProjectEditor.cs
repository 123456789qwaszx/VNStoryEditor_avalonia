...
public sealed partial class ProjectEditor
{
    ...
    /// <summary>
    /// 이 노드가 <b>더 이상 어느 에피소드도 아니게</b> 한다 — 에피소드가 지워졌는데 연출이
    /// 들어 있어 노드는 남겨 둘 때의 자리다. 돌려주는 값은 새 이름이다.
    ///
    /// ⛔ <b>표식만 비우면 안 된다.</b> <see cref="EpisodeNaming.EpisodeIdOf"/>는 표식이 없으면
    /// <b>이름을 뒷길로</b> 쓴다. [＋ 에피소드]가 세운 노드는 <i>이름이 곧 Id</i>라서, 표식만
    /// 비운 노드는 여전히 그 에피소드라고 말한다 — 유령이 그대로 남고, 같은 Id로 에피소드를
    /// 다시 만들면 판에 같은 이름이 둘이 되어 <b>내보내기가 "번들 이름이 겹칩니다"로 통째로
    /// 막힌다</b>. 2026-08-25에 고쳤다고 적어 둔 그 사고이고, 2026-09-17까지 <b>이름이 Id와
    /// 같은 경우에는 안 고쳐져 있었다</b>(화면이 필드를 직접 비우고 있었다).
    ///
    /// ⚠ 이름은 <b>사람이 볼 표시</b>이지 신원이 아니다. 여기서 붙이는 꼬리표는 "정해 주세요"를
    /// 판에서 눈에 띄게 하려는 것이고, 사람이 바로 고쳐도 된다.
    /// </summary>
    public string DetachEpisodeMark(string nodeId)
    {
        if (Project.FindNode(nodeId) is not DialogueNode node)
        {
            return string.Empty;
        }

        StoryFile? board = Project.Files.FirstOrDefault(file => file.Nodes.Contains(node));
        string detached = FreeDetachedName(board, node.Name);
    ...
    /// <summary>
    /// 떼어 낸 노드에 줄 이름. 판에서 <b>안 겹치는</b> 것을 고른다 — 번들 이름이 노드 이름에서
    /// 나오므로 겹치면 내보내기가 막힌다(고치려는 사고를 다른 모양으로 되풀이하는 셈이다).
    /// </summary>
    private static string FreeDetachedName(StoryFile? board, string name)
    {
        const string Suffix = " (떼어냄)";
        ...
        bool Taken(string candidate) => board.Nodes.Any(other =>
            string.Equals(other.Name, candidate, StringComparison.Ordinal));

        if (!Taken(name + Suffix))
        {
            return name + Suffix;
        }
        ...
        {
            string candidate = $"{name} (떼어냄 {number})";
