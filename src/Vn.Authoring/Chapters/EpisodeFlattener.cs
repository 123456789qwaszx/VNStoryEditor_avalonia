...
/// <param name="EmittedLineIds">산출된 순서대로의 LineId. 중복이 없음을 증명하는 근거다.</param>
public sealed record EpisodeFlattenResult(
    string Text,
    IReadOnlyList<string> EmittedLineIds,
...
{
    /// <param name="conditionExpressions">챕터 `조건` 시트의 라벨 → 식 (G-7).</param>
    public static EpisodeFlattenResult Flatten(
        EpisodeWorkbookModel model,
        IReadOnlyDictionary<string, string> conditionExpressions)
    {
        ArgumentNullException.ThrowIfNull(model);
        ArgumentNullException.ThrowIfNull(conditionExpressions);
        ...
        for (int position = 0; position < mainFlow.Count; position++)
        {
            ...
            switch (row.Kind)
            {
                case EpisodeRowKind.If:
                    EmitCondition(model, row, position, mainFlow, conditionExpressions,
                ...
                default:
                    EmitDialogue(row, builder, emitted, indent: 0);
                    break;
            }
        }
    ...
    {
        string? expression = Expression(model, row, conditionExpressions, diagnostics);
        ...
        {
            EmitDialogue(inner, builder, emitted, indent: 1);
        }
        ...
        List<ChapterDiagnostic> diagnostics)
    {
        ...
        if (!conditionExpressions.TryGetValue(label, out string? expression) ||
