...
///
/// <b>읽기만 한다.</b> 폴더를 만들지도, 파일을 쓰지도 않는다.
///
/// ⛔ <b>읽는 이유가 2026-09-16(R-F)에 바뀌었다.</b> 옛 머리글은 <i>"이 레이어에서 엑셀은
/// 언제나 사람이 소유하는 원본이고 툴은 소비자다(G-2)"</i>였다. 그 전제가 뒤집혔다 —
/// 저작의 주인은 <see cref="ChapterDocument"/>이고 <c>chapters/*.xlsx</c>는 <b>산출물</b>이다.
///
/// 그래서 <see cref="Load"/>를 부르는 자리는 이제 <b>하나뿐</b>이다:
/// <c>ChapterImportService</c> — 프로젝트 <i>밖에서 들어온</i> 워크북을 모델로 들이는 길.
/// 나머지 호출자가 쓰는 것은 경로 헬퍼(<see cref="FolderFor"/>)이지 읽기가 아니다.
///
/// ⚠ <b>제품 경로에서 이 클래스로 챕터를 읽지 말 것.</b> 그러면 정본이 둘이 되고, 엑셀에서
/// 고친 값과 프로젝트의 값이 조용히 갈린다 — 뒤집기가 없애려던 사고가 정확히 그것이다.
///
