...
///
/// <b>무엇을 보나</b> — 문법과 <b>전역 라인 ID 유일성</b>(계약서 C4)까지다. 어휘(미등록
/// 커맨드)는 보지 않는다: 그 사전이 <c>game.definition.json</c>과 <c>game.schema.json</c>
/// 둘로 갈려 있어, 여기서 한쪽을 고르면 <b>세 번째 판정 기준</b>이 생긴다. 저작 쪽이 이미
/// 커맨드를 정의 파일로 검사하므로 그 둘을 한 어휘로 합칠 때까지 여기서는 묻지 않는다.
///
