...
public class OutputManifestTests
{
    ...
    public void 노드_이름을_바꾸면_옛_파일이_고아로_보인다()
    {
        ...
        try
        {
            ...
            Assert.Contains("테스트_파일_본문.yarn", Directory.GetFiles(directory).Select(Path.GetFileName));
            ...
            Assert.Equal(["테스트_파일_본문.yarn"], scan.Orphans.Select(orphan => orphan.FileName));
            ...
            Assert.Contains("테스트_파일_본문개정.yarn", onDisk);
            Assert.Contains("테스트_파일_본문.yarn", onDisk);
        }
        finally
        {
