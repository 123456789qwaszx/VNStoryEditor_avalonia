...
- **대사 본문·화자** → `episodes/{챕터}/{Id}.xlsx` — 원본은 엑셀이고, 대량 작업은 거기서 합니다.
  연출 그래프의 카드에서 **[🔒 대사 잠김]을 풀면** 화자·내용만 도구에서도 고칠 수 있고,
  고친 값은 **그 엑셀 셀에 바로 저장**됩니다(2026-08-24). 줄 추가·삭제·조건 블록은 엑셀에서만.
- **에피소드 구조·간선·조건·스탯** → `chapters/{Id}.xlsx` — 도구 편집이 셀에 즉시 저장됩니다.
  ⚠ **간선은 [연출 그래프]에서도 긋습니다**(2026-09-17, R7): 카드 아래 3칸에 문구를 적어
  다른 카드로 끌면 그것이 간선입니다. **창구가 둘이지 값이 사는 곳은 하나**라, 어느 쪽에서
  만들어도 [챕터 그래프]에 같은 간선으로 보이고 내보내기·검증도 그대로입니다.
- ⚠ **엑셀이 그 파일을 열고 있으면** 도구 편집이 **저절로 잠기고** 붉은 배너가 섭니다.
  엑셀에서 닫으면 저절로 풀립니다 — 켜고 끄는 체크는 없습니다(2026-08-24).
